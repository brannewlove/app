...
CREATE TABLE `confirmed_assets` (
    ...
    `asset_number` varchar(100) COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
    `cj_id` varchar(50) COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '사원ID(확인 시점의 사용자)',
    ...
    KEY `idx_cj_id` (`cj_id`)
) ENGINE = InnoDB AUTO_INCREMENT = 18 DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '사용자 변경 확인 테이블';
...
CREATE TABLE `trade` (
    ...
    CONSTRAINT `trade_ibfk_1` FOREIGN KEY (`cj_id`) REFERENCES `users` (`cj_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT `trade_ibfk_2` FOREIGN KEY (`asset_number`) REFERENCES `assets` (`asset_number`) ON DELETE RESTRICT ON UPDATE RESTRICT

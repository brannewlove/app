...
3. **거래 취소 및 기록 보존 (Rollback & Audit)**:
   - 거래 취소 시 물리적 `DELETE`를 수행하지 않고, `trade` 테이블의 `is_cancelled = 1` 및 `cancelled_at` 시간을 기록하여 감사 추적(Audit Trail)을 보존합니다.
   - **신규 등록 자산(`신규-계약`, `신규-고장교체`, `신규-기타`) 취소 시**: 신규로 등록되었던 해당 자산(`assets` 레코드)을 완전히 삭제합니다.
   - **기타 모든 거래 취소 시**: 직전 자산 상태(`asset_state`), 직전 소유자(`asset_in_user` 또는 `ex_user`), 메모(`asset_memo`)를 기반으로 `assets` 테이블 정보를 거래 발생 전 원래 상태로 안전하게 복원합니다.
   - 데이터 무결성 보장을 위해 최신 거래만 취소 가능하며, 이미 취소된 거래는 중복 취소되지 않습니다.
